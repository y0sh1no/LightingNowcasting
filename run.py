...
def create_parser():
    ...
    parser.add_argument("--save_every_epochs", type=int, default=1, help="save checkpoint every N epochs (1 to save every epoch)")
...
class Runner(object):
    ...
    def _load_data(self):
        ...
        self.data_type       = 'lighting' if self.args.dataset == 'lighting' else 'vil'
        
        # Use standard PyTorch DataLoader for all datasets (STPDataset returns vil sequence tensors)
    ...
    def load(self, milestone):
        ...
        # milestone can be a filepath (string containing '.pt'), an int, or a numeric string
        if isinstance(milestone, str) and '.pt' in milestone:
            path = milestone
        elif isinstance(milestone, int):
            path = osp.join(self.ckpt_path, f"ckpt-{milestone}.pt")
        elif isinstance(milestone, str) and milestone.isdigit():
            path = osp.join(self.ckpt_path, f"ckpt-{milestone}.pt")
        else:
            # fallback to treating as filename fragment
            path = osp.join(self.ckpt_path, f"ckpt-{milestone}.pt")
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.global_epochs):
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                # step-based saving removed; checkpoints saved per-epoch only
                        ...
                        try:
                            ...
                            if self.is_main:
                                for i in range(radar_ori.shape[0]):
                                    self.visiual_save_fn(radar_recon[i], radar_ori[i], osp.join(self.sanity_path, f"{i}/vil"),data_type=self.data_type)

                        except Exception as e:
                            print_log(e, self.is_main)
                            print_log("Sanity Check Failed", self.is_main)

            # save checkpoint and do test every epoch (or every N epochs if configured)
            if self.args.save_every_epochs > 0 and ((epoch + 1) % self.args.save_every_epochs == 0):
                self.save()
    ...
    def _train_batch(self, batch):
        ...
        _, loss = self.model.predict(frames_in=frames_in, frames_gt=frames_out, compute_loss=True, T_out=self.args.frames_out)
        if loss is None:
    ...
    def test_samples(self, milestone, do_test=False):
        ...
        for batch in tqdm(data_loader,desc='Test Samples', disable=not self.is_main):
            ...
            if self.is_main:
                for i in range(radar_ori.shape[0]):
                    self.visiual_save_fn(radar_recon[i], radar_ori[i], osp.join(save_dir, f"{cnt}-{i}/vil"),data_type=self.data_type, save_grays=True, save_colored=True)
